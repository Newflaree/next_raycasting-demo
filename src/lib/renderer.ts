...
export function renderScene(
  ...
  // === RENDERIZADO DE MUROS, TECHO Y PISO ===
  for (let i = 0; i < rays.length; i++) {
    ...
    const shade = Math.max(0, 255 - correctedDistance * light);
    const wallColor = `rgb(${shade}, ${shade}, ${shade})`;
    const floorColor = `rgb(${shade * 0.5}, ${shade * 0.5}, ${shade * 0.5})`;
    const ceilColor = `rgb(${shade * 0.2}, ${shade * 0.2}, ${shade * 0.2})`;
  ...
  for (const sprite of visibleSprites) {
    ...
    if (img && img.complete && img.naturalWidth !== 0) {
      ...
      console.warn(`⚠️ Imagen no cargada o inválida: ${texture}`);
    }
  }
